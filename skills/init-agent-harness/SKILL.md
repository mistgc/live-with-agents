...
The AGENTS.md section written by this skill points at both, so the AGENT reads them (or the relevant part) whenever it files, retrieves, or classifies a note.
...
3. **Install the governing reference docs.** Rules 2 and 3 below are defined authoritatively by two documents, so install both into the repo's `docs/` (create the folder if missing) before the AGENTS.md rules reference them:
   ...
   Copy the current authoritative copies of the two files, keeping both in the same folder so their relative cross-links (`state-lifecycle.md` ↔ `project-notes-schema.md`) keep resolving. Source them from the reference scaffold when the repo carries it — the `project-structure-with-agents` template ships both under its `docs/` — otherwise from wherever this skill's authoritative copies live. If the repo already has identical copies, skip; if a local copy diverges (user-customized or a newer edition), confirm before overwriting rather than clobbering it.
