...
description: MANUAL-ONLY skill. Do not auto-invoke. Use ONLY when the user directly asks to run this skill — e.g. to set up or refresh a project's AGENTS.md with output-constraint rules, adding a .agents harness so agents store generated docs (plan/spec/prd/reports) under git-ignored .agents/local, file concluded work as typed notes (implemented/deprecated/fixed/rejected/archived) under .agents/notes, and consult those notes when answering repo questions. It also installs the governing reference docs — docs/state-lifecycle.md and docs/project-notes-schema.md — into the repo's docs/, writes a README under .agents/notes explaining that folder and pointing at both docs by relative path, and the AGENTS.md rules it writes tell future agents to read them when filing or consulting notes.
...
Rules 2 and 3 rest on two authoritative reference documents, which the skill installs into the project so a future agent can look up the exact model:

- `docs/state-lifecycle.md` — where notes come from: the promotion gate, the five types on their two tracks (current-truth / history / terminal).
- `docs/project-notes-schema.md` — the note schema (`project-notes/v2`): frontmatter fields, the filename rule, conformance checks.

The AGENTS.md section written by this skill points at both, so the AGENT reads them (or the relevant part) whenever it files, retrieves, or classifies a note. This skill bundles authoritative copies of both under its own `references/` directory, so it can install them without reaching outside itself, and it writes a short `.agents/notes/README.md` that orients a reader who lands in the notes folder and links both docs by relative path into that `references/` directory.
...
3. **Install the governing reference docs and the notes README.** Rules 2 and 3 below are defined authoritatively by two documents, so install both into the repo's `docs/` (create the folder if missing) before the AGENTS.md rules reference them:
   - `docs/state-lifecycle.md` — the state lifecycle: promotion gate, five types on two tracks, where each type is filed.
   - `docs/project-notes-schema.md` — the note schema (`project-notes/v2`): frontmatter fields, filename rule, conformance.
   Copy the current authoritative copies of the two files from this skill's own `references/` directory (bundled next to this SKILL.md), keeping both in the same folder so their relative cross-links (`state-lifecycle.md` ↔ `project-notes-schema.md`) keep resolving. When the repo carries the `project-structure-with-agents` template, its `docs/` copies are byte-identical, so copying from there is equally fine. If the repo already has identical copies, skip; if a local copy diverges (user-customized or a newer edition), confirm before overwriting rather than clobbering it.

   Then write a short `README.md` into the repo's `.agents/notes/`, so anyone who lands in the notes folder is oriented. It explains what `.agents/notes` is and links both docs by **relative path into the skill's own reference directory** — installed, this skill sits at `.agents/skills/init-agent-harness/`, so from `.agents/notes/` the links are `../skills/init-agent-harness/references/<doc>` (the docs' skill-relative path `references/<doc>`, anchored by the skill's install location). Use this body (adjust only if the repo's style clearly differs):
...
5. **Write the three rules into AGENTS.md** as a section (create it now if the file was empty). Append below, adjusting heading to the file's existing structure. Idempotency: if a rule with the same intent already exists, replace it rather than adding a duplicate section. The block points note semantics at `docs/state-lifecycle.md` and `docs/project-notes-schema.md` (installed in Step 3), so it stays short here instead of restating the schema.
   ...
   ### The note model is defined in docs/ — read it when it matters
   `docs/state-lifecycle.md` (promotion gate, five types on two tracks) and `docs/project-notes-schema.md` (frontmatter fields, filename rule, conformance) are the authoritative reference for everything below about notes. When the AGENT files, retrieves, or classifies a note — or answers from one and the type↔track distinction or a field matters — it reads the relevant part of these two files instead of relying on this short summary.
...
6. **Encode the filename convention.** Generated working documents under `.agents/local/` follow `<datetime>-<topic>.md` — a leading date (ISO, e.g. `2026-09-05`), then the topic, joined by `-`, e.g. `2026-09-05-rag-eval-spec.md`. Concluded-work notes follow the schema's own filename rule — `<date>-<topic>.md`, the date equal to the frontmatter `date` (see `docs/project-notes-schema.md`) — the same shape, with the schema doc authoritative for notes. Without a date prefix, working docs of the same topic collide and notes can't be ordered by when the work was done.

7. **Verify.** Re-read the final AGENTS.md; confirm all three rules are present and readable, and that every rule referencing `docs/state-lifecycle.md` / `docs/project-notes-schema.md` is backed by both files actually existing in `docs/` (installed in Step 3). Confirm `.agents/notes/README.md` exists and its two relative links resolve to the skill's bundled copies under `.agents/skills/init-agent-harness/references/`. Confirm `.agents/local` content is ignored (`git check-ignore .agents/local/<any doc>` reports it) and the scaffold folders exist. Show the resulting section to the user.
...
| AGENTS.md rules name `docs/state-lifecycle.md` / `docs/project-notes-schema.md` but the files were never installed | Step 3 installs both under `docs/`; Step 7 verifies they exist before the rules may reference them |
| The notes README is written but the docs it links were never installed | Step 3 installs both docs; the README's links into `.agents/skills/init-agent-harness/references/` must resolve — Step 7 checks them |
...
| Copying only one of the two reference docs into `docs/` | Keep both in the same folder — their relative links (`state-lifecycle.md` ↔ `project-notes-schema.md`) depend on it |
| Clobbering a repo's customized copy of a reference doc | Copy only when missing or identical; confirm before replacing a diverged local edition |
